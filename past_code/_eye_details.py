...
def compute_mm_per_px(image, scale_method: str = 'eye'):
    """
    Computes the mm/px scale based on the selected method (eye width or sticker).
    """
    h, w, _ = image.shape
    ...
    if scale_method == 'sticker':
        mm_from_sticker, _ = calculate_scale_from_sticker(image, lm, REFERENCE_STICKER_MM)
        if mm_from_sticker is not None:
            return mm_from_sticker
        else:
            print("[INFO] Sticker not found. Falling back to eye-width scale.")

    return MEASURED_EYE_WIDTH_MM / eye_width_px if eye_width_px > 0 else None
...
def analyze_eye_details(image, mm_per_px):
    """
    目の開き具合を、上瞼・下瞼それぞれ独立してmm単位で計測する関数
    """
    h, w, _ = image.shape
    rgb_image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    results = face_mesh.process(rgb_image)
    ...
    def dist_point_to_line(point, line_start, line_end):
        """
        点(point)から、始点と終点を結ぶ直線への垂直距離を計算する
        (直線より上がプラス、下がマイナスになるように符号を調整)
        """
        # 直線ベクトル
    ...
    print("\n" + "="*60)
    print(f"{'EYE':<6} | {'LID':<6} | {'STATUS':<20} | {'DIST(mm)':<8}")
    print("-" * 60)

    eyes = [
        ("LEFT", L_OUTER, L_INNER, L_TOP, L_BOTTOM),
        ("RIGHT", R_OUTER, R_INNER, R_TOP, R_BOTTOM)
    ]

    for side, outer_id, inner_id, top_id, bottom_id in eyes:
        p_outer = get_pt(outer_id)
        p_inner = get_pt(inner_id)
        p_top = get_pt(top_id)
        p_bottom = get_pt(bottom_id)
        
        # 1. 基準線（目尻と目頭を結ぶ線）からの距離を計算
        # ※ MediaPipeのY座標は下がプラスなので、計算結果の符号に注意
        # ここでは「基準線より上(上瞼)」をプラス、「下(下瞼)」をプラスとして扱うためabsをとる
        
        # 上瞼の距離 (基準線 〜 上瞼)
        top_dist_px = abs(dist_point_to_line(p_top, p_outer, p_inner))
        
        # 下瞼の距離 (基準線 〜 下瞼)
        bottom_dist_px = abs(dist_point_to_line(p_bottom, p_outer, p_inner))
        
        # 合計開瞼幅 (Total Eye Opening)
        total_open_px = top_dist_px + bottom_dist_px
        
        # mm換算
        top_mm = top_dist_px * mm_per_px
        bottom_mm = bottom_dist_px * mm_per_px
        total_mm = total_open_px * mm_per_px

        # --- 描画 ---
        # 基準線（青）
        cv2.line(image, tuple(p_outer.astype(int)), tuple(p_inner.astype(int)), (255, 255, 0), 1)
        # 上瞼の距離（赤線）
        cv2.line(image, tuple(p_top.astype(int)), (int(p_top[0]), int(p_top[1] + top_dist_px)), (0, 0, 255), 1) # 簡易描画
        # 下瞼の距離（赤線）
        cv2.line(image, tuple(p_bottom.astype(int)), (int(p_bottom[0]), int(p_bottom[1] - bottom_dist_px)), (0, 0, 255), 1) # 簡易描画

        # --- 評価コメント ---
        # 完全に閉じている判定 (閾値は仮で2mm以下など)
        state = "OPEN"
        if total_mm < 2.0:
            state = "CLOSED"
        elif total_mm < 5.0:
        ...
        print(f"{side:<6} | UPPER  | Dist from center     | {top_mm:6.2f} mm")
        print(f"{'':<6} | LOWER  | Dist from center     | {bottom_mm:6.2f} mm")
        print(f"{'':<6} | TOTAL  | {state:<20} | {total_mm:6.2f} mm")
        print("-" * 60)
        
        # 画像への書き込み
        label_pos = p_top - [10, 20]
        info = f"U:{top_mm:.1f} L:{bottom_mm:.1f} T:{total_mm:.1f}"
        cv2.putText(image, info, tuple(label_pos.astype(int)), 
...
if original_img is not None:
    ...
    if aligned_img is not None:
        ...
        result_img = analyze_eye_details(aligned_img, mm_per_px=compute_mm_per_px(aligned_img, scale_method=scale_method))
